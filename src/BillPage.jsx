            ...
            <div style={{ marginTop: 15, flex: 1, display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
                    <div style={{ width: '48%' }}>
                        ...
                        <div style={{ fontSize: 16, color: '#1e293b', fontWeight: 600 }}>{bill.phone}</div>
                    </div>
                ...
                <table className="premium-print-table">
                    <thead>
                        <tr>
                            ...
                            <th style={{ textAlign: 'left' }}>{t('description', lang).toUpperCase()}</th>
                            <th style={{ textAlign: 'right', width: '65px' }}>{t('qty', lang).toUpperCase()}</th>
                            <th style={{ textAlign: 'right', width: '90px' }}>{t('unit_price', lang).toUpperCase()}</th>
                            <th style={{ textAlign: 'right', width: '110px' }}>{t('total', lang).toUpperCase()}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {items.map((item, idx) => {
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 15 }}>
                    <div style={{ width: '340px', fontSize: 14.5, border: '3px double #a1a1a1', padding: '12px 16px', background: '#ffffff' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 6, fontSize: 13, color: '#475569', borderTop: '1px solid #e2e8f0', marginTop: 4 }}>
                            ...
                            <span style={{ fontWeight: 800, textTransform: 'uppercase' }}>{resolvedMethod}</span>
                        </div>
                    </div>
                </div>
            ...
            <div className="alive-footer-lock">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 25, marginTop: 20 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', width: '220px', borderTop: '3px double #a1a1a1', paddingTop: 8 }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 800, color: '#334155' }}>Receiver's Signature</span>
                    </div>
                    {bill.vehicleNo && (
                        <div style={{ textAlign: 'center' }}>
                            <img 
                                ...
                                alt="QR Record" 
                            />
                            <div style={{ fontSize: 8, fontWeight: 800, marginTop: 3, color: '#64748b' }}>VERIFIED RECORD</div>
                        </div>
                    )}
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', width: '220px', borderTop: '3px double #a1a1a1', paddingTop: 8 }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 800, color: '#334155' }}>Authorized Signature</span>
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'stretch', fontSize: '13px', color: '#1e293b', marginTop: 10 }}>
                    ...
                    <div style={{ textAlign: 'right', maxWidth: '38%', lineHeight: 1.4, display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                        <span style={{ fontWeight: 850, fontSize: '12px', color: '#334155' }}>Dua for Traveling: "Bismillahi majreha wa mursaha, inna Rabbi la-Ghafurur Rahim."</span>
                    </div>
                </div>
            </div>
        </div>,
...
const BillDraftForm = ({ draft, onUpdate, savedBills, inventory, services, setServices, customers, payments, setSavedBills, setInventory, setPayments, updateCustomerLedger, logActivity, jobCards, setJobCards, onPrintReady, companies, generateSafeId, returnToInventory, jobIntakes }) => {
    ...
    const handleFinalizeBill = () => {
        ...
        logActivity(`Bill/Cash Memo ${newInvoiceId} created for ${customerName}`);
            ...
            <Row gutter={24}>
                <Col xs={24} lg={16}>
                    {jobIntakes?.length > 0 && (
                        <div style={{ marginBottom: 24, padding: 15, background: 'rgba(212,175,55,0.05)', border: '1px solid rgba(212,175,55,0.2)', borderRadius: 4 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <Text style={{ color: '#3B82F6', fontWeight: 900, fontSize: 12 }}>PENDING REQUESTS ({jobIntakes.length})</Text>
                    ...
                    <div className="glass-card billing-card-wrapper" style={{ padding: '25px', marginBottom: 24, background: '#1E293B', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <Row gutter={16}>
                            <Col span={12}>
                                <Form.Item label={<span style={{ color: '#FFFFFF', fontWeight: 900, fontSize: '14px' }}>Customer/Company</span>} required>
                            ...
                            <Col span={12}>
                                <Form.Item label={<span style={{ color: '#FFFFFF', fontWeight: 900, fontSize: '14px' }}>Contact Phone</span>} required>
                                    <SmartVoiceInput value={phone} onChange={v => set('phone', v)} className="glass-input premium-glow-input" placeholder="e.g. 01712345678" />
                                </Form.Item>
                            </Col>
                            <Col span={8}>
                                <Form.Item label={<span style={{ color: '#FFFFFF', fontWeight: 900, fontSize: '14px' }}>Vehicle No</span>}>
                            ...
                            <Col span={16}>
                                <Form.Item label={<span style={{ color: '#FFFFFF', fontWeight: 900, fontSize: '14px' }}>Billing Entity</span>}>
                                    <Radio.Group value={billingEntity} onChange={e => set('billingEntity', e.target.value)}>
                                        <Radio.Button value="Mamun Automobiles" style={{ fontWeight: 800 }}>MAMUN AUTOMOBILES</Radio.Button>
                                        <Radio.Button value="Muntaha Motors" style={{ fontWeight: 800 }}>MUNTAHA MOTORS</Radio.Button>
                                    </Radio.Group>
                                </Form.Item>
                            </Col>
                        </Row>
                    ...
                    <div className="glass-card billing-card-wrapper" style={{ padding: '25px', background: '#1E293B', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, borderBottom: '2px solid rgba(59,130,246,0.3)', paddingBottom: '10px' }}>
                            ...
                            <Space>
                                <Button size="small" onClick={() => setIsServiceModalVisible(true)} style={{ background: '#3B82F6', color: '#FFFFFF', fontWeight: 850, border: 'none' }}>+ SERVICE</Button>
                                <Input 
                        ...
                        <Button type="dashed" block onClick={() => setItems([...safeLineItems, { id: generateSafeId('BLANK'), description: '', quantity: 1, unitPrice: 0 }])} style={{ marginTop: 16, fontWeight: 800, borderColor: '#3B82F6', color: '#3B82F6' }}>
                            ADD BLANK ROW
                        </Button>
                    </div>
                ...
                <Col xs={24} lg={8}>
                    <div className="glass-card valuation-panel" style={{ padding: '30px', position: 'sticky', top: 16, background: '#1E293B', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <div style={{ lineHeight: 2.5 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <Text strong style={{ color: '#FFFFFF', fontWeight: 900, fontSize: '13px' }}>SUBTOTAL</Text>
                                <Text style={{ color: '#FFFFFF', fontWeight: 900, fontSize: '14px' }}>৳{subtotal.toLocaleString()}</Text>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                ...
                                <Text style={{ color: '#FFFFFF', fontWeight: 900, fontSize: '14px' }}>+ ৳{vatAmount.toLocaleString()}</Text>
                            </div>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(34, 197, 94, 0.15)', padding: '16px 20px', borderRadius: '12px', border: '2.5px solid #22c55e', marginTop: '12px', boxShadow: '0 4px 20px rgba(34, 197, 94, 0.25)' }}>
                                <span style={{ color: '#22c55e', fontWeight: 950, fontSize: '20px', letterSpacing: '1px' }}>TOTAL</span>
                                <span style={{ color: '#22c55e', fontWeight: 950, fontSize: '32px', textShadow: '0 2px 12px rgba(34, 197, 94, 0.5)' }}>৳{netPayable.toLocaleString()}</span>
                            </div>
                        </div>
                        <Divider style={{ borderColor: 'rgba(255,255,255,0.1)' }} />
                        <Form.Item label={<span style={{ color: '#FFFFFF', fontWeight: 900, fontSize: '14px' }}>Payment Method</span>}>
                            <Radio.Group value={draft.paymentMethod} onChange={e => set('paymentMethod', e.target.value)}>
                                <Radio.Button value="Cash" style={{ fontWeight: 800 }}>CASH</Radio.Button>
                                <Radio.Button value="Bank" style={{ fontWeight: 800 }}>BANK</Radio.Button>
                                <Radio.Button value="Bkash" style={{ fontWeight: 800 }}>BKASH</Radio.Button>
                            </Radio.Group>
                        </Form.Item>
