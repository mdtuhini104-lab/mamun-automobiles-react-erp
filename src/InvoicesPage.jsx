// v1.6.0 - Premium Invoices Module (Anti-Gravity Layout)
import React from 'react';
import { bufferedPrint } from './utils/printAssistant';
import { createPortal } from 'react-dom';
import { Card, Table, Button, Input, Select, DatePicker, Row, Col, Typography, InputNumber, Modal, Form, Space, message, Radio, Divider, Tabs, AutoComplete, Tag, Checkbox, Avatar, Alert, Tooltip, List } from 'antd'; } from 'antd'; } from 'antd';
    ...
    return createPortal(
        <div className="alive-print-area">
            ...
            <BrandedDocumentHeader 
                ...
                meta={[
                    { label: 'Invoice No', value: bill.id },
...
const BillDraftForm = ({ draft, onUpdate, savedBills, inventory, services, setServices, customers, payments, setSavedBills, setInventory, setPayments, updateCustomerLedger, logActivity, isAdmin, jobCards, setJobCards, onPrintReady, companies, generateSafeId, returnToInventory, isDark, jobIntakes }) => {
    ...
    const handleFinalizeBill = () => {
        ...
        logActivity(`Invoice ${newInvoiceId} created for ${customerName}`);
        ...
        <div className="no-print" style={{ marginTop: 20 }}>
            <Row gutter={24}>
                <Col xs={24} lg={16}>
                    ...
                    <div className="glass-card" style={{ padding: '25px', marginBottom: 24, background: '#1E293B', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <Title level={4} style={{ color: '#3B82F6', fontWeight: 600, marginBottom: 20 }}>Client Details</Title>
                        <Row gutter={16}>
                            <Col span={12}>
                                <Form.Item label="Customer/Company" required>
                                    <AutoComplete options={unifiedClientOptions} onSelect={handleClientSelect} onChange={v => set('customerName', v)} value={customerName}>
                                        <SmartVoiceInput className="glass-input" />
                                    </AutoComplete>
                                </Form.Item>
                            </Col>
                            <Col span={12}>
                                <Form.Item label="Contact Phone" required>
                                    <SmartVoiceInput value={phone} onChange={v => set('phone', v)} className="glass-input" />
                                </Form.Item>
                            </Col>
                            <Col span={8}>
                                <Form.Item label="Vehicle No">
                                    <SmartVoiceInput value={vehicleNo} onChange={v => set('vehicleNo', v)} className="glass-input" />
                                </Form.Item>
                            </Col>
                            <Col span={16}>
                                <Form.Item label="Billing Entity">
                    ...
                    <div className="glass-card" style={{ padding: '25px', background: '#1E293B', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
                            <Title level={4} style={{ color: '#3B82F6', margin: 0, fontWeight: 600 }}>Items & Services</Title>
                            <Space>
                        ...
                        <Button type="dashed" block onClick={() => setItems([...safeLineItems, { id: generateSafeId('BLANK'), description: '', quantity: 1, unitPrice: 0 }])} style={{ marginTop: 16, fontWeight: 600 }}>
                            Add Blank Row
                        </Button>
                    </div>

                    <div className="glass-card" style={{ padding: '25px', marginTop: 24, background: '#1E293B', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <Title level={4} style={{ color: '#3B82F6', marginBottom: 20, fontWeight: 600 }}>Signatures & Verification</Title>
                        <Row gutter={32}>
                            <Col span={12}>
                                <SignatureField 
                ...
                <Col xs={24} lg={8}>
                    <div className="glass-card" style={{ padding: '30px', position: 'sticky', top: 16, background: '#1E293B', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <Title level={4} style={{ color: '#3B82F6', marginBottom: 24, fontWeight: 600 }}>Valuation</Title>
                        <div style={{ lineHeight: 2.5 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <Text strong style={{ fontWeight: 600 }}>Subtotal</Text>
                                <Text style={{ fontWeight: 600 }}>৳{subtotal.toLocaleString()}</Text>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <Text strong style={{ fontWeight: 600 }}>Discount</Text>
                                <InputNumber min={0} value={discount} onChange={v => set('discount', v || 0)} />
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <Text strong style={{ fontWeight: 600 }}>VAT (5%)</Text>
                                <Text style={{ fontWeight: 600 }}>+ ৳{vatAmount.toLocaleString()}</Text>
                            </div>
                            <Divider />
                        ...
                        <Form.Item label="Paid Amount">
                            <InputNumber min={0} value={paidAmount} onChange={v => set('paidAmount', v || 0)} style={{ width: '100%' }} />
                        </Form.Item>
                        <Button 
                            type="primary" 
                            block 
